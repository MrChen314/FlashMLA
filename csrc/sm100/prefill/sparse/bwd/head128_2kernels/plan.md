...
- 实现范围只考虑 `topk == 128`，不处理其他 `topk` 规模。
...
- `S / dS` 的 staging 按 paired-topk tile 设计：
  ...
  - 当前实现只支持 `topk == 128`，因此只有一个固定的 paired tile
  - `local_k_block = cta_idx`
...
- `dkv_phase.cuh` 的 kernel 结构按下面固定：
  ...
  - kernel 只支持 `topk == 128`，因此不做 `num_k_pairs` 循环，直接执行单个 fixed pair
  - 两个 CTA 分别 TMA 取 `S` 和 `dS` 的一个 `64`-row half，不需要 odd-tail 清零分支
  ...
  - 行号映射固定为 `row_global = cta_idx * 64 + row`
...
- dKV kernel 当前只支持 `params.topk == 128`，不处理其他 `topk` 规模
