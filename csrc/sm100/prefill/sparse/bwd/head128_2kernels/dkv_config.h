...
static constexpr int TOPK_GRANULARITY = 256;
...
static constexpr int NUM_S_DS_BUFS = 1;
static constexpr int NUM_THREADS = 12 * 32;

static_assert(DKV_TILE_M == 2 * B_H, "dKV paired tile expects a 256-row MMA tile split across two CTAs.");
static_assert(DKV_ROWS_PER_CTA == B_H, "Each CTA in the dKV kernel owns 128 rows.");
...
static_assert(NUM_S_DS_BUFS == 1, "dKV kernel currently expects ping-pong shared-memory buffering for S/dS tiles.");
...
using TiledMMA_dKV_Part0 = decltype(make_tiled_mma(
    SM100_MMA_F16BF16_2x1SM_SS_NOELECT<bf16, bf16, float, 256, 256, UMMA::Major::MN, UMMA::Major::MN>{}
));

using TiledMMA_dKV_Part1_2 = decltype(make_tiled_mma(
    SM100_MMA_F16BF16_2x1SM_SS_NOELECT<bf16, bf16, float, 256, 128, UMMA::Major::MN, UMMA::Major::MN>{}
));

using TiledMMA_dKV_RoPE = decltype(make_tiled_mma(
    SM100_MMA_F16BF16_2x1SM_SS_NOELECT<bf16, bf16, float, 256, 64, UMMA::Major::MN, UMMA::Major::MN>{}
));
...
static_assert(tmem_cols::kNumUsedCols == 512, "dKV kernel should fully use the 512 logical TMEM columns.");
...
struct alignas(128) SharedMemoryPlan {
    ...
    transac_bar_t bar_dkv_part0_ready[NUM_S_DS_BUFS];
    transac_bar_t bar_dkv_rope_ready[NUM_S_DS_BUFS];
    transac_bar_t bar_dkv_part1_ready[NUM_S_DS_BUFS];
    transac_bar_t bar_dkv_part2_ready[NUM_S_DS_BUFS];
