...
class Bwd_Sm100_Head128_2KernelsFusedImpl : public BwdImplBase {
...
struct SparseBwdHead128_2KernelsFusedLaunchContext {
    int h_q;
    int d_qk;
...
static SparseBwdHead128_2KernelsFusedLaunchContext prepare_sparse_attn_bwd_head128_2kernels_fused_context(
    const at::Tensor &q,
    const at::Tensor &kv,
    const at::Tensor &o,
    const at::Tensor &dO,
...
static std::vector<BwdFeatures> build_sparse_bwd_head128_2kernels_fused_required_features(
    int h_q,
    int d_qk,
    bool have_topk_length
) {
...
static std::vector<at::Tensor> sparse_attn_bwd_head128_2kernels_fused_interface(
    const at::Tensor &q,
    const at::Tensor &kv,
    const at::Tensor &o,
    const at::Tensor &dO,
    ...
    auto ctx = prepare_sparse_attn_bwd_head128_2kernels_fused_context(
        q, kv, o, dO, indices, lse,
        sm_scale, d_v, topk_length, q_start_index_s,
        arch.num_sms,
        at::cuda::getCurrentCUDAStream().stream()
    );
    auto required_features = build_sparse_bwd_head128_2kernels_fused_required_features(
        ctx.h_q, ctx.d_qk, ctx.have_topk_length);

    if (ctx.h_q == 128) {
        Bwd_Sm100_Head128_2KernelsFusedImpl bwd_impl;
    ...
    return {ctx.dQ, ctx.dKV.to(torch::kBFloat16)};
}
