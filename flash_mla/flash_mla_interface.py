...
def flash_mla_sparse_bwd(
    ...
    Args:
        q: [s_q, h_q, d_qk], bfloat16 - Query张量
        kv: [s_kv, h_kv, d_qk], bfloat16 - Key/Value张量
        o: [s_q, h_q, d_v], bfloat16 - 前向输出
        dO: [s_q, h_q, d_v], bfloat16 - 输出梯度
        indices: [s_q, h_kv, topk], int32 - TopK索引
        lse: [s_q, h_q], float32 - Log-Sum-Exp (来自前向)
        sm_scale: float - Softmax缩放因子
        d_v: int - Value维度，必须为512
