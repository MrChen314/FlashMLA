...
def test_sparse_mla_bwd_head128_2kernels_fused(
    B=1,
    S=64,
    SKV=256,
    H=128,
    ...
    if cc_major != 10:
        pytest.skip("head128_2kernels fused path currently targets SM100 only")
    ...
    flash_dq, flash_dkv = flash_mla.flash_mla_sparse_bwd_head128_2kernels_fused(
    ...
    def fn():
        return flash_mla.flash_mla_sparse_bwd_head128_2kernels_fused(
            q_3d, kv_3d, flash_out, do_3d, indices_3d, flash_lse,
            sm_scale=sm_scale,
            q_start_index_s=q_start_index_s,
        )
    ...
    if kk.is_using_profiling_tools():
        ...
        has_dq_kernel = any("dq_phase_kernel" in name for name in kernel_names)
        has_dkv_kernel = any("dkv_phase_kernel" in name for name in kernel_names)
        has_preprocess_kernel = any("preprocess_delta" in name for name in kernel_names)

        if has_preprocess_kernel and has_dkv_kernel:
            bwd_time_s = bench_result.get_e2e_time("preprocess_delta", "dkv_phase_kernel")
        elif has_dq_kernel and has_dkv_kernel:
            bwd_time_s = bench_result.get_e2e_time("dq_phase_kernel", "dkv_phase_kernel")
        elif len(kernel_names) > 0:
            bwd_time_s = bench_result.get_e2e_time(kernel_names[0], kernel_names[-1])
        else:
            raise RuntimeError("No CUDA kernels were captured by bench_kineto for flash_mla_sparse_bwd_head128_2kernels_fused")
...
if __name__ == "__main__":
    test_sparse_mla_bwd_head128_2kernels_fused(
        B=1,
        S=4096,
        SKV=8192,
        H=128,
